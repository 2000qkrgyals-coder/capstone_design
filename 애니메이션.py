...
NPZ_PATH = BASE_DIR / "trajectory_1min_only.npz"
...
def draw_badge(d, xy, text, font, pad=(14, 8), radius=12):
    x, y = xy
    pad_x, pad_y = pad
...
start = st.slider("Start Time", 0, T - 1, 540)
end = st.slider("End Time", 0, T - 1, 600)
...
if coords is not None and len(coords) > 0:
    for x, y in coords:
        ...
        ys = int(y * scale)
        draw.ellipse(
            (xs - POINT_RADIUS, ys - POINT_RADIUS,
             xs + POINT_RADIUS, ys + POINT_RADIUS),
            fill=POINT_COLOR
        )
